...
def get_same_or_newer(start_date):
    ...
    while start_date < today_date:
        ...
        for row in reader:
            ...
            if row_date in emp_StartDate_dict.keys():
                emp_StartDate_dict[min_date].append(min_date_employees)
            else :
                emp_StartDate_dict[min_date] = min_date_employees

        start_date = start_date + datetime.timedelta(days=1)

    return emp_StartDate_dict

def list_newer(start_date):
        for start_date, employees  in get_same_or_newer(start_date).items():
